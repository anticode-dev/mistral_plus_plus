...
class ToolManager:
    ...
    async def _integrate_mcp_async(self) -> None:
        try:
            ...
            # Register MCP servers from installed plugins
            plugin_count = await self._register_plugin_mcp_servers()

            total = http_count + stdio_count + plugin_count
            if total > 0:
                logger.info(
                    "MCP integration registered %d tools (http=%d, stdio=%d, plugins=%d)",
                    total,
                    http_count,
                    stdio_count,
                    plugin_count,
    ...
    async def _register_plugin_mcp_servers(self) -> int:
        """Register MCP servers provided by installed plugins."""
        try:
            from vibe.core.plugins import PluginManager
        except ImportError:
            return 0
        ...
        try:
            ...
            for plugin in pm.list_installed():
                if not plugin.enabled:
                    continue
                if not plugin.manifest or not plugin.manifest.mcp_server:
                    continue

                mcp_config = plugin.manifest.mcp_server
                ...
                if transport == "stdio":
                    ...
                    if not full_cmd:
                        logger.warning(
                            "Plugin '%s' MCP server has empty command", plugin.name
                        )
                        continue

                    try:
                        tools: list[RemoteTool] = await list_tools_stdio(full_cmd)
                        for remote in tools:
                            proxy_cls = create_mcp_stdio_proxy_tool_class(
                                command=full_cmd,
                                remote=remote,
                                alias=plugin.name,
                                server_hint=mcp_config.get("prompt"),
                            )
                            self._available[proxy_cls.get_name()] = proxy_cls
                            added += 1
                    except Exception as exc:
                        logger.warning(
                            "Failed to register plugin '%s' MCP tools: %s",
                            plugin.name,
                            exc,
                        )
            ...
            logger.warning("Failed to load plugin MCP servers: %s", exc)
            return 0
