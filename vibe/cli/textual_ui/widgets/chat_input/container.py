...
class ChatInputContainer(Vertical):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        command_entries = self._command_registry.completion_entries()
        self._completion_manager = self._build_completion_manager(command_entries)
    ...
    def refresh_commands(self) -> None:
        command_entries = self._command_registry.completion_entries()
        self._completion_manager = self._build_completion_manager(command_entries)
        if self._body and self._body.input_widget:
            self._body.set_completion_reset_callback(self._completion_manager.reset)
            self._body.input_widget.set_completion_manager(self._completion_manager)
            self._completion_manager.on_text_changed(
                self._body.input_widget.text, self._body.input_widget.get_cursor_offset()
            )

    def _build_completion_manager(
        self, command_entries: list[tuple[str, str]]
    ) -> MultiCompletionManager:
        return MultiCompletionManager([
