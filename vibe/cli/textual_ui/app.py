...
from vibe.cli.commands import CommandRegistry, PluginCommand as CLIPluginCommand
...
from vibe.core.performance import high_speed_coroutine
from vibe.core.plugins.manager import PluginManager
from vibe.core.plugins.models import (
    InstalledPlugin,
    PluginAgent,
    PluginCommand as PluginManifestCommand,
)
...
class VibeApp(App):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._plugin_command_sources: dict[
            str, tuple[InstalledPlugin, PluginManifestCommand]
        ] = {}
        self._plugin_agents: dict[str, dict[str, PluginAgent]] = {}
    ...
    def _ensure_plugin_manager(self) -> PluginManager | None:
        ...
        try:
            ...
            logger.warning("Failed to initialize PluginManager: %s", exc)
            return None

    def _load_plugin_data(self) -> None:
        pm = self._ensure_plugin_manager()
        if pm is None:
            return

        try:
            self._plugin_command_sources = {
                alias.lower(): value for alias, value in pm.get_plugin_commands().items()
            }

            registry_commands: list[CLIPluginCommand] = []
            self._plugin_agents = {}

            for plugin in pm.list_installed():
                ...
                if agents:
                    self._plugin_agents[plugin.name.lower()] = agents

            self.commands.set_plugin_commands(registry_commands)
            self._refresh_command_completions()
        except Exception as exc:
            logger.warning("Failed to load plugin data: %s", exc)

    def _refresh_command_completions(self) -> None:
        if self._chat_input_container:
            self._chat_input_container.refresh_commands()

    async def _handle_command(self, user_input: str) -> bool:
        if command := self.commands.find_command(user_input):
            handler = getattr(self, command.handler)
            if asyncio.iscoroutinefunction(handler):
                # Pass full input for commands that need arguments
                ...
                handler()
            return True
        if plugin_command := self.commands.find_plugin_command(user_input):
            await self._handle_plugin_invocation(plugin_command, user_input)
            return True
        return False

    def _format_plugin_subagents(self, plugin_name: str) -> str | None:
        agents = self._plugin_agents.get(plugin_name.lower())
        if not agents:
            return None

        lines = [
            "# Available subagents",
            "Use `/subagent <plugin> <agent> [input]` to run one of these prompts.",
        ]
    ...
    async def _handle_plugin_invocation(
        self, plugin_command: CLIPluginCommand, user_input: str
    ) -> None:
        pm = self._ensure_plugin_manager()
        if pm is None:
            await self._mount_and_scroll(
                ErrorMessage(
                    "Plugin manager unavailable. Try restarting Vibe.",
                    collapsed=self._tools_collapsed,
                )
            )
            return

        base_alias, args = self.commands.parse_command_args(user_input)
        alias = base_alias.lower()

        if alias not in self._plugin_command_sources:
            self._load_plugin_data()

        source = self._plugin_command_sources.get(alias)
        if not source:
            await self._mount_and_scroll(
                ErrorMessage(
                    "Unknown plugin command. Run `/reload` or reinstall the plugin.",
                    collapsed=self._tools_collapsed,
                )
            )
            return

        plugin, manifest_cmd = source
        prompt = pm.get_command_prompt(plugin, manifest_cmd)
        if not prompt:
            await self._mount_and_scroll(
                ErrorMessage(
                    f"Prompt not found for `{plugin.name}:{manifest_cmd.name}`.",
                    collapsed=self._tools_collapsed,
                )
            )
            return
        ...
        user_args = " ".join(args)
        message_to_send = (
            f"[Plugin Command: {plugin.name}:{manifest_cmd.name}]\n\n{prompt}"
        )

        if user_args:
            message_to_send = f"{message_to_send}\n\n# User Input\n{user_args}"

        if subagents := self._format_plugin_subagents(plugin.name):
            message_to_send = f"{message_to_send}\n\n{subagents}"

        await self._handle_user_message(user_input, agent_message=message_to_send)
    ...
    async def _handle_user_message(
        self, message: str, agent_message: str | None = None
    ) -> None:
        outbound_message = agent_message or message
        init_task = self._ensure_agent_init_task()
        pending_init = bool(init_task and not init_task.done())
        
        # If agent is running, queue the message instead of processing immediately
        if self._agent_running and self.agent:
            self.agent.queue_message(outbound_message)
            user_message = UserMessage(message, pending=True, queued=True)
        ...
        self.run_worker(
            self._process_user_message_after_mount(
                message=message,
                agent_message=outbound_message,
        ...
        try:
            ...
            if self.agent and not self._agent_running:
                self._agent_task = asyncio.create_task(
                    self._handle_agent_turn(agent_message)
                )
    ...
    async def _process_queued_messages(self) -> None:
        ...
        if not self.agent or not self.agent.has_queued_messages():
            return
        ...
        try:
            while self.agent.has_queued_messages():
                async for event in self.agent.process_next_queued_message():
    ...
    async def _handle_agent_turn(self, prompt: str) -> None:
        ...
        try:
            ...
            if self.agent and self.agent.has_queued_messages():
...
"""
    ...
    async def _run_subagent(self, user_input: str = "") -> None:
        pm = self._ensure_plugin_manager()
        if pm is None:
            await self._mount_and_scroll(
                ErrorMessage(
        ...
        _, args = self.commands.parse_command_args(user_input)
        if len(args) < 2:
            await self._mount_and_scroll(
                ErrorMessage(
                    "Usage: /subagent <plugin> <agent> [input]",
                    collapsed=self._tools_collapsed,
                )
            )
            return

        plugin_name, agent_name = args[0], args[1]
        user_args = " ".join(args[2:]) if len(args) > 2 else ""

        plugin = pm.get_plugin(plugin_name)
        if not plugin:
            plugin = next(
                (p for p in pm.list_installed() if p.name.lower() == plugin_name.lower()),
                None,
            )
        if not plugin or not plugin.enabled or not plugin.manifest:
            await self._mount_and_scroll(
                ErrorMessage(
                    f"Plugin `{plugin_name}` not found or disabled.",
                    collapsed=self._tools_collapsed,
                )
            )
            return
        ...
        if plugin_key not in self._plugin_agents:
            self._load_plugin_data()

        agents = self._plugin_agents.get(plugin_key)
        if not agents:
            await self._mount_and_scroll(
                ErrorMessage(
                    f"Plugin `{plugin.name}` has no subagents available.",
                    collapsed=self._tools_collapsed,
                )
            )
            return
        ...
        if not agent:
            available = ", ".join(sorted(a.name for a in agents.values()))
            await self._mount_and_scroll(
                ErrorMessage(
        ...
        prompt = pm.get_agent_prompt(plugin, agent)
        if not prompt:
            await self._mount_and_scroll(
                ErrorMessage(
                    f"Prompt not found for subagent `{agent.name}`.",
                    collapsed=self._tools_collapsed,
                )
            )
            return

        message_to_send = f"[Subagent: {plugin.name}/{agent.name}]\n\n{prompt}"
        if user_args:
            message_to_send = f"{message_to_send}\n\n# User Input\n{user_args}"

        await self._handle_user_message(user_input, agent_message=message_to_send)

    async def _handle_plugin_command(self, user_input: str = "") -> None:
        """Handle /plugin commands."""
        _, args = self.commands.parse_command_args(user_input)

        if not args:
            # Show plugin help
            help_text = """## Plugin Management

**Usage:** `/plugin <subcommand> [args]`

**Subcommands:**
- `install <repo>` - Install plugin from GitHub (e.g., `EveryInc/compound-engineering-plugin`)
- `remove <name>` - Remove an installed plugin
- `list` - List installed plugins
- `enable <name>` - Enable a disabled plugin
- `disable <name>` - Disable a plugin
- `marketplace add <url>` - Add a plugin marketplace
- `marketplace list` - List added marketplaces
- `marketplace remove <url>` - Remove a marketplace
        ...
        try:
            ...
            match subcommand:
                case "install":
                    if not subargs:
                        await self._mount_and_scroll(
                            ErrorMessage("Usage: /plugin install <repo>", collapsed=self._tools_collapsed)
                        )
                        return
                    repo = subargs[0]
                    await self._mount_and_scroll(UserCommandMessage(f"Installing plugin from `{repo}`..."))
                    plugin = await pm.install(repo)
                    msg = f"""## Plugin Installed
...
Plugin commands and subagents are ready to use."""
                ...
                case "remove" | "uninstall":
                    if not subargs:
                        await self._mount_and_scroll(
                            ErrorMessage("Usage: /plugin remove <name>", collapsed=self._tools_collapsed)
                        )
                        return
                    name = subargs[0]
                    if pm.uninstall(name):
                        await self._mount_and_scroll(UserCommandMessage(f"Plugin `{name}` removed."))
                        self._load_plugin_data()
                    else:
                        await self._mount_and_scroll(
                            ErrorMessage(f"Plugin `{name}` not found.", collapsed=self._tools_collapsed)
                        )
                ...
                case "enable":
                    if not subargs:
                        await self._mount_and_scroll(
                            ErrorMessage("Usage: /plugin enable <name>", collapsed=self._tools_collapsed)
                        )
                        return
                    name = subargs[0]
                    if pm.enable(name):
                        await self._mount_and_scroll(UserCommandMessage(f"Plugin `{name}` enabled."))
                        self._load_plugin_data()
                    else:
                        await self._mount_and_scroll(
                            ErrorMessage(f"Plugin `{name}` not found.", collapsed=self._tools_collapsed)
                ...
                case "disable":
                    if not subargs:
                        await self._mount_and_scroll(
                            ErrorMessage("Usage: /plugin disable <name>", collapsed=self._tools_collapsed)
                        )
                        return
                    name = subargs[0]
                    if pm.disable(name):
                        await self._mount_and_scroll(UserCommandMessage(f"Plugin `{name}` disabled."))
                        self._load_plugin_data()
                    else:
                        await self._mount_and_scroll(
                            ErrorMessage(f"Plugin `{name}` not found.", collapsed=self._tools_collapsed)
                ...
                case "marketplace":
                    if not subargs:
                        await self._mount_and_scroll(
                            ErrorMessage("Usage: /plugin marketplace <add|list|remove> [url]", collapsed=self._tools_collapsed)
                        )
                        return

                    mp_cmd = subargs[0].lower()
                    mp_args = subargs[1:] if len(subargs) > 1 else []

                    match mp_cmd:
                        case "add":
                            if not mp_args:
                                await self._mount_and_scroll(
                                    ErrorMessage("Usage: /plugin marketplace add <url>", collapsed=self._tools_collapsed)
                                )
                                return
                            url = mp_args[0]
                            await self._mount_and_scroll(UserCommandMessage(f"Adding marketplace `{url}`..."))
                            mp = await pm.add_marketplace(url)
                            msg = f"Marketplace **{mp.name}** added with {len(mp.plugins)} plugins."
                            await self._mount_and_scroll(UserCommandMessage(msg))

                        case "list":
                            mps = pm.list_marketplaces()
                            if not mps:
                                await self._mount_and_scroll(UserCommandMessage("No marketplaces added."))
                            else:
                                lines = ["## Plugin Marketplaces\n"]
                                for mp in mps:
                                    lines.append(f"- **{mp.name}** ({mp.url})")
                                    if mp.plugins:
                                        lines.append(f"  {len(mp.plugins)} plugins available")
                                await self._mount_and_scroll(UserCommandMessage("\n".join(lines)))

                        case "remove":
                            if not mp_args:
                                await self._mount_and_scroll(
                                    ErrorMessage("Usage: /plugin marketplace remove <url>", collapsed=self._tools_collapsed)
                                )
                                return
                            url = mp_args[0]
                            if pm.remove_marketplace(url):
                                await self._mount_and_scroll(UserCommandMessage("Marketplace removed."))
                            else:
                                await self._mount_and_scroll(
                                    ErrorMessage("Marketplace not found.", collapsed=self._tools_collapsed)
                                )

                        case _:
                            await self._mount_and_scroll(
                                ErrorMessage(f"Unknown marketplace command: {mp_cmd}", collapsed=self._tools_collapsed)
                            )

                case _:
                    await self._mount_and_scroll(
                        ErrorMessage(f"Unknown plugin command: {subcommand}. Use `/plugin` for help.", collapsed=self._tools_collapsed)
                    )

        except Exception as e:
            await self._mount_and_scroll(
                ErrorMessage(f"Plugin command failed: {e}", collapsed=self._tools_collapsed)
            )
