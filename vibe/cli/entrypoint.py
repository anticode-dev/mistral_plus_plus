...
# Parse FPS early BEFORE importing textual (which reads TEXTUAL_FPS at import time)
# This enables --target-fps to actually take effect
def _configure_fps_early() -> None:
    """Set TEXTUAL_FPS from args or defaults before Textual imports."""
    for i, arg in enumerate(sys.argv):
        if arg == "--target-fps" and i + 1 < len(sys.argv):
            try:
                fps = int(sys.argv[i + 1])
                os.environ["TEXTUAL_FPS"] = str(fps)
                return
            except ValueError:
                pass
        elif arg.startswith("--target-fps="):
            try:
                ...
                pass
    # Default to 120 FPS for high performance mode
    os.environ.setdefault("TEXTUAL_FPS", "120")

_configure_fps_early()
...
def parse_arguments() -> argparse.Namespace:
    ...
    performance_group.add_argument(
        "--performance-mode",
        type=str,
        choices=["balanced", "high", "extreme"],
        default="high",
        help="Performance mode: 'high' for optimized performance (default), "
        "'balanced' for normal operation, 'extreme' for maximum speed.",
    )
    performance_group.add_argument(
        "--target-fps",
        type=int,
        default=None,
        help="Target frames per second for UI rendering (default: 120 in high mode). "
        "Set to 300 for ultra-smooth terminals. Use TEXTUAL_FPS env var to override globally.",
    )
...
def main() -> None:  # noqa: PLR0912, PLR0915
    ...
    if args.performance_mode:
        os.environ["VIBE_PERFORMANCE"] = args.performance_mode

    if args.benchmark:
        from vibe.core.benchmark import run_comprehensive_benchmark
